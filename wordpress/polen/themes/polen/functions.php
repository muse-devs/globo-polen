...
function polen_set_message($type, $message)
{
	$_SESSION['polen-messages']['type'] = $type;
	$_SESSION['polen-messages']['message'] = $message;
}

function polen_clear_messages()
{
	unset($_SESSION['polen-messages']);
}

function write_message_to_js()
{
	$msg = $_SESSION['polen-messages']['message'];
	return "\"{$msg}\"";
}

function polen_has_message()
{
	if( isset($_SESSION['polen-messages'])) {
		return true;
	}
	return false;
}
...
add_filter('woocommerce_add_error', function($message){
	polen_set_message('error', $message);
	return null;
}, 10, 1);

add_action('polen_messages_service', function() {
	if(polen_has_message()):
?>
<script>
polError(<?= write_message_to_js(); ?>);
</script>
<?php
		polen_clear_messages();
	endif;
}, 10, 0 );

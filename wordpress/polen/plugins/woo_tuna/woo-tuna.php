...
class TUNA_Payment extends WC_Payment_Gateway
{
    ...
    public function payment_pix_verify()
    {
        ...
        switch (strval($response->status)) {
            ...
            case '2':
                ...
                $customer_order->update_status('approved', __('Tuna Payments: Pagamento confirmado.', 'tuna-payment'));
    ...
    public function payment_callback()
    {
        ...
        switch (strval($status)) {
            ...
            case '2':
                // Changing the order for processing and reduces the stock.
                $customer_order->update_status('wc-payment-approved', __('Tuna Payments: Pagamento confirmado.', 'tuna-payment'));
                wc_reduce_stock_levels((int) $orderID);
                break;
            case 'C':
            case 'P':
    ...
    public function init_form_fields()
    {
        $this->form_fields = array(
            'title' => array(
                ...
                'default'	=> __('Cartão de Crédito, Pix ou Boleto', 'tuna-payment'),
            ),
            'description' => array(
                'title'		=> __('Descrição', 'tuna-payment'),
                'type'		=> 'textarea',
                'desc_tip'	=> __('Descrição que o usuário vai ver na tela de pagamento.', 'tuna-payment'),
                'default'	=> __('Pague com seu cartão de crédito, pix ou boleto, através da Tuna.', 'tuna-payment'),
                'css'		=> 'max-width:350px;'
            ),
